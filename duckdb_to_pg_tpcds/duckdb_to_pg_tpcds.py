...
output_rows = []
empty_files = []  # <- to record empty files
id_counter = 1

# ==== Main processing loop ====
for template in range(1, 23): # 1 23
    for query in range(1, 2): # 1 101
        print(f"template:{template}, query:{query}")

        found_valid_run = False
        for run in range(0, 51):
            filename = f"{template}_{query}_run{run}.json"
        ...
        if not found_valid_run:
            print(f"All 51 runs empty for template {template}, query {query}")
            empty_files.append((template, query))
